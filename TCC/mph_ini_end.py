...
filename = f"moon_phases_{start_year}-{end_year - 1}_UTC_no_lib.csv"
with open(filename, 'w', newline='') as csvfile:
    fieldnames = ['Date', 'Area', 'Category', 'Moon Phase', 'Phase']
        ...
        for month in range(1, 13):
            for day in range(1, month_max_days(month, year) + 1):
                ...
                writer.writerow({
                    ...
                    'Category': category,
                    'Moon Phase': MOON_ICONS[category],
                    'Phase': category if category < 5 else 8 - category
                })
